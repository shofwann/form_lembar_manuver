...
fileInput.forEach(input => { 
    input.addEventListener('change', e => { 
        ...
        // const nama = e.target.name.trimEnd();
        // const newName = e.target.name = nama;
        // console.log(newName);
    }); 
});
...
function tambahFormUpdate(index,namaTitel,namaLokasi,namaInstallasi,namaId,namaFoto,lokasiAppendForm,lokasiAppendRow,clasRemoveFormBottonBebas,clasNewContainer,clasBtn,clasExContainer){ //0,'titelBebas[]','lokasiManuverBebas[]','installManuverBebas[]','idBebas[]','copyFormBebas','rowBebas'
    ...
    for (i=1; i<btnExist.length; i++){
        btnExist[i].children[0].style.display = 'none';
    }
...
function kurangFormUpdate(ini,clasNewContainer,clasExContainer){
    let newCont = document.querySelectorAll(`.${clasNewContainer}`);
    const jumCont = newCont.length;
    console.log(jumCont)

    row = ini.parentElement.parentElement;

    // const div = ini.parentElement.parentElement.parentElement.parentElement.parentElement;
    // const existingForm = div.querySelectorAll(`.${clasExContainer}`);
    // existingForm.forEach((row, idx) => {
    //     row.children[1].childNodes[3].innerText = idx + 1;
    // }); 

    if (jumCont == 1){
        ini.parentElement.parentElement.parentElement.parentElement.previousElementSibling.children[2].innerHTML = `<button type="button"  class="btn reed" onclick="kurangFormUpdateExist(this,'${clasExContainer}')">-</button>`;
        ...
        //console.log(ini.parentElement.parentElement.parentElement.previousElementSibling.children[0].children[2])//children[0].children[1].children[0]).style.display= '' ;
        ini.parentElement.parentElement.parentElement.previousElementSibling.children[0].children[2].innerHTML = `<button type="button" class="btn reed" onclick="kurangFormUpdate(this,'${clasNewContainer}','${clasExContainer}')">-</button>`;
        row.remove()
    } 
...
function kurangFormUpdateExist(ini,clasExContainer){
    let containerExis = document.querySelectorAll(`.${clasExContainer}`)
    const jumCont = containerExis.length;
    console.log(jumCont)

    if (jumCont == 2) {
        ini.parentElement.parentElement.remove();
    } else {
        ini.parentElement.parentElement.previousElementSibling.children[2].innerHTML = `<button type="button" class="btn reed" onclick="kurangFormUpdateExist(this,'${clasExContainer}')">-</button>`;
        ini.parentElement.parentElement.remove();
    }
...
for(i=1; i<btnBebas.length; i++){
    btnBebas[i].children[0].style.display = 'none';
    btnBebas[btnBebas.length-1].innerHTML = `<button type="button" class="btn reed" onclick="kurangFormUpdateExist(this,'exist-bebas')">-</button>`;
}

let btnNormal = document.querySelectorAll('.btn_normal');
for(i=1; i<btnNormal.length; i++){
    btnNormal[i].children[0].style.display = 'none';
    btnNormal[btnNormal.length-1].innerHTML = `<button type="button" class="btn reed" onclick="kurangFormUpdateExist(this,'exist-normal')">-</button>`;
}
